...
Inicio el exploit con la ``url`` y la flag ``-c`` para que ejecute un comando ``whoami`` dentro de la maquina victima.
...
El resultado es exitos vemos el usuario ``iusr`` dce la maquina victima.
...
Para abrir el puerto usare ``netsh`` esto solo funciona cuando somos administrador, otra forma es con el comando ``net share`` para crear un recurso compartido.
